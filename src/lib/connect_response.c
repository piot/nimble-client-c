...
int nimbleClientOnConnectResponse(NimbleClient* self, FldInStream* inStream)
{
    ...
    if (self->state == NimbleClientStateRequestingConnect) {
        ...
        connectionLayerIncomingInit(&self->connectionLayerIncoming, response.connectionIdSecret);
        connectionLayerOutgoingInit(&self->connectionLayerOutgoing, response.connectionIdSecret);
